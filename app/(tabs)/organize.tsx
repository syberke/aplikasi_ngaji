...
import { ChartBar as BarChart3, Calendar, Copy, Eye, Plus, Settings, Trash2, UserMinus, UserPlus, Users, X } from 'lucide-react-native';
...
const [showLeaveModal, setShowLeaveModal] = useState(false);
  ...
  const leaveOrganize = async () => {
  if (!organize || !profile) return;

  // 1. keluarkan semua siswa
  await supabase
    .from('users')
    .update({ organize_id: null })
    .eq('organize_id', organize.id);
...
const joinOrganizeByCode = async () => {
  if (!joinCode.trim()) {
    Alert.alert('Error', 'Kode kelas wajib diisi');
    return;
  }
  ...
  if (error || !organizeData) {
    Alert.alert('Error', 'Kode kelas tidak ditemukan');
    return;
  }
...
await refreshProfile(); // 🔥
  ...
  Alert.alert('Berhasil', `Berhasil bergabung ke kelas ${organizeData.name}`);
            ...
            <View style={styles.noOrganizeContainer}>
              ...
              >
                ...
                <Text style={styles.createFirstButtonText}>Buat Kelas</Text>
                  ...
                  <Pressable
  style={[styles.createFirstButton, { backgroundColor: '#10B981', marginTop: 12 }]}
  onPress={() => setShowJoinForm(true)}
>
  <Text style={styles.createFirstButtonText}>Gabung Kelas</Text>
</Pressable>
{showJoinForm && (
  <View style={styles.createForm}>
    <Text style={styles.formTitle}>Gabung Kelas</Text>
    ...
    <View style={styles.formActions}>
      ...
      <Pressable
        style={styles.createButton}
        onPress={joinOrganizeByCode}
      >
        <Text style={styles.createButtonText}>Gabung</Text>
      </Pressable>
    </View>
  </View>
)}
...
>
  <View style={styles.modalOverlay}>
    <View style={styles.modalContent}>
      <Text style={styles.modalTitle}>Keluar dari Kelas</Text>
