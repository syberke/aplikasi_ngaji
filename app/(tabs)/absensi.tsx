  ...
  const fetchStudentsInOrganize = async () => {
  ...
  const fetchAttendanceData = async () => {
    if (!profile || !profile.organize_id) return;
  ...
  // ortu / guru
    ...
    if (selectedStudentId) {
      ...
      const hadirCount = data?.filter(r => r.status === 'hadir').length || 0;
      const izinCount = data?.filter(r => r.status === 'izin').length || 0;
      const tidakHadirCount = data?.filter(r => r.status === 'tidak_hadir').length || 0;
      ...
      console.error('Error fetchAttendanceData:', error);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
        ...
        {students.map(student => (
          <Pressable
            ...
            style={{
              ...
              marginRight: 8,
              backgroundColor:
                selectedStudentId === student.id ? '#8B5CF6' : '#E5E7EB',
            }}
          >
            <Text
              style={{
                color:
                  selectedStudentId === student.id ? 'white' : '#1F2937',
                fontWeight: '600',
              }}
            >
              {student.name}
